...
class DownloadAPK:
    def __init__(self, save_dir=None, thread_num=3, fpath_pre_urls='', log_download='', use_proxy=False):
        self.use_proxy = use_proxy  # 如果在国内使用,需要设置代理
        self.headers = {
    ...
    def extract_pkg_and_download(self, pkg):
        ...
        client = httpx.Client(http2=True, verify=False, proxies=self.proxies if self.use_proxy else {})
...
if __name__ == '__main__':
    ...
    downloader = DownloadAPK(save_dir=output_dir, thread_num=threads, fpath_pre_urls=fpath_apk_urls,
                             log_download=fpath_download_log, use_proxy=True)
